...
import {VALIDATION_MESSAGE} from "@/app/configs/validation_msg";
import {REGEX_EMAIL, REGEX_PHONE_NUMBER_SIMPLE} from "@/app/configs/constants";
...
export const numberRequired = yup.number().required(VALIDATION_MESSAGE.REQUIRED).typeError(VALIDATION_MESSAGE.ONLY_NUMBER).nullable();
export const numberNotPositiveRequired = yup.number().required(VALIDATION_MESSAGE.REQUIRED).min(0, VALIDATION_MESSAGE.NOT_NUMBER_POSITIVE).typeError(VALIDATION_MESSAGE.ONLY_NUMBER);
export const stringRequiredLimitSmall = yup
    .string()
    .required(VALIDATION_MESSAGE.REQUIRED)
    .nullable();
export const stringRequiredLimitBig = yup
    .string()
    .required(VALIDATION_MESSAGE.REQUIRED)
    .nullable();
export const arrayStringNormal = yup
    .array()
    // .of(yup.string())
    .nullable();export const arrayStringRequired = yup
    .array()
    // .of(yup.string())
    .nullable()
    .min(0)
    .required(VALIDATION_MESSAGE.REQUIRED);
export const numberLimitBig = yup
    .number()
    .required(VALIDATION_MESSAGE.REQUIRED)
    .min(1, VALIDATION_MESSAGE.REQUIRED);
